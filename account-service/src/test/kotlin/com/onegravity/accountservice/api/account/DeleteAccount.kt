...
class DeleteAccount : BehaviorSpec( {
    testApplication { testEngine ->
        ...
        `when`("I call DELETE /api/v1/admin/accounts/{accountUUID}") {
            val call = testEngine.handleRequest(HttpMethod.Delete, "/api/v1/admin/accounts/${newAccount!!.accountUUID}")
            val updatedAccount = gson.fromJson(call.response.content.toString(), Account::class.java)
        ...
        `when`("I call DELETE /api/v1/admin/accounts/{accountUUID} with an invalid uuid") {
